            ...
            pcall(function()
                ...
                for _, root in ipairs(roots) do
                    ...
                    if jobIDLabel then
                        ...
                        for _, v in ipairs(container:GetDescendants()) do
                            if v:IsA("TextButton") and v.Text:match("Join Job%-ID") then tempButton = v;
                            break end
                        end
                ...
                if tempTextbox and tempButton then chilliTextbox = tempTextbox;
                chilliButton = tempButton end
            end)
...
local COLORS = {
    ...
    SectionBG = Color3.fromRGB(35, 15, 15),
    Stroke = Color3.fromRGB(255, 0, 0), 
    TextWhite = Color3.fromRGB(255, 255, 255),
    MoneyText = Color3.fromRGB(100, 255, 150),
    Selected = Color3.fromRGB(220, 0, 0), 
    Unselected = Color3.fromRGB(55, 20, 20),
    ForceActive = Color3.fromRGB(255, 50, 50),
    AntiLagActive = Color3.fromRGB(255, 0, 0),
    ForgeBlue = Color3.fromRGB(220, 0, 0),
...
local PET_DATABASE = {
    ...
    ["Secret"] = {
        ...
        "Los Mobilis", "Los Nooo My Hotspotsitos", "Los Planitos", "Los Quesadillas", "Los Spaghettis", 
        ...
        "Trickolino", "Tung Tung Tung Sahur", "Vulturino Skeletono", "W or L", "Luck Luck Luck Sahur", 
        "Zombie Tralala"
    },
    ["OG"] = {
...
local function applyAnimatedGradient(obj, colors)
    if not obj:IsA("TextLabel") and not obj:IsA("TextButton") and not obj:IsA("TextBox") and not obj:IsA("UIStroke") and not obj:IsA("Frame") then return end
...
local function destroyGarbage(item)
    ...
    if item:IsA("Accessory") or item:IsA("Shirt") or item:IsA("Pants") or item:IsA("ShirtGraphic") or item:IsA("CharacterMesh") or item:IsA("Hat") or item:IsA("FaceControls") then
        item:Destroy()
...
ScreenGui.Name = "Miku_v1_SecureWebhook"
...
local function createOverlayLabel(text)
    ...
    stroke.Parent = frame
    applyAnimatedGradient(stroke, GRADIENT_AQUA)
    return frame, lbl
end
...
RunService.RenderStepped:Connect(function() if MainFrame.Visible then StrokeGradient.Rotation = StrokeGradient.Rotation + 3 end end)
...
Title.Parent = Header
applyAnimatedGradient(Title, GRADIENT_AQUA)

local FilterBtn = Instance.new("TextButton")
FilterBtn.Size = UDim2.new(0, 80, 0, 28)
FilterBtn.Position = U
